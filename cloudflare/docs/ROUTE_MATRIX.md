...
| GET | `/api/public/appointment` | pública com capability token | query `demoId` | `X-Appointment-Token`; rate limit | `Tenant`, `TenantSettings`, `AppointmentAccessToken`, `Appointment`, `Service`, `Professional` | atualiza último uso quando aplicável | resumo com negócio, terminologia e capacidades | `ManageAppointment` | `P/appointment` |
...
| POST | `/api/public/appointment/cancel` | pública com capability token | query `demoId` | `X-Appointment-Token`; confirmação obrigatória; rate limit | token, settings, `Appointment`, `AppointmentHistoryEvent`, slots | cancela, registra motivo/histórico, preserva token e libera slots atomicamente | resumo público cancelado | `ManageAppointment` | `P/appointment/cancel` |
...
| POST | `/api/public/appointment/reschedule` | pública com capability token | query `demoId` | `X-Appointment-Token`; confirmação obrigatória; rate limit | token, settings, disponibilidade, `Appointment`, slots e histórico | atualiza o mesmo agendamento, preserva token e troca slots atomicamente | 201 resumo + mesmo `managementPath` | `ManageAppointment` | `P/appointment/reschedule` |
